-- ========================================
-- SCRIPT SQL - CRÉATION TABLES CRM
-- Pour société de transport et logistique
-- ========================================
...
CREATE INDEX idx_leads_company ON crm_leads(company);
CREATE INDEX idx_leads_status ON crm_leads(status);
CREATE INDEX idx_leads_assigned_to ON crm_leads(assigned_to);
CREATE INDEX idx_leads_country ON crm_leads(country);
CREATE INDEX idx_leads_created_at ON crm_leads(created_at);
...
CREATE INDEX idx_opportunities_assigned_to ON crm_opportunities(assigned_to);
CREATE INDEX idx_opportunities_expected_close_date ON crm_opportunities(expected_close_date);
CREATE INDEX idx_opportunities_value ON crm_opportunities(value);
...
-- Index
CREATE INDEX idx_quotes_quote_number ON crm_quotes(quote_number);
CREATE INDEX idx_quotes_status ON crm_quotes(status);
CREATE INDEX idx_quotes_created_by ON crm_quotes(created_by);
CREATE INDEX idx_quotes_valid_until ON crm_quotes(valid_until);
...
-- Index
CREATE INDEX idx_quote_items_quote_id ON crm_quote_items(quote_id);
...
CREATE INDEX idx_activities_lead_id ON crm_activities(lead_id);
CREATE INDEX idx_activities_opportunity_id ON crm_activities(opportunity_id);
...
-- Index
CREATE INDEX idx_activity_participants_activity_id ON crm_activity_participants(activity_id);
CREATE INDEX idx_activity_participants_personnel_id ON crm_activity_participants(personnel_id);
...
-- Index
CREATE INDEX idx_pipeline_stages_pipeline_id ON crm_pipeline_stages(pipeline_id);
...
-- Index
CREATE INDEX idx_attachments_lead_id ON crm_attachments(lead_id);
CREATE INDEX idx_attachments_opportunity_id ON crm_attachments(opportunity_id);
CREATE INDEX idx_attachments_quote_id ON crm_attachments(quote_id);
CREATE INDEX idx_attachments_activity_id ON crm_attachments(activity_id);
...
-- Vue: Résumé des prospects par commercial
...
-- Pipeline par défaut
INSERT INTO crm_pipelines (name, description, is_default, created_by) VALUES
('Pipeline Standard Transport', 'Pipeline par défaut pour les opportunités de transport', true, 1);
...
(1, 'Analyse des besoins', 'Étude détaillée des besoins transport', '#fd7e14', 3, 50, 'needs_analysis'),
(1, 'Proposition/Devis', 'Envoi de la proposition commerciale', '#6f42c1', 4, 75, 'proposal'),
...
(1, 'Perdu', 'Opportunité non convertie', '#dc3545', 6, 0, 'closed_lost');

-- Tags par défaut
INSERT INTO crm_tags (name, color, category, created_by) VALUES
('Transport Express', '#dc3545', 'lead', 1),
('Transport National', '#007bff', 'lead', 1),
('Transport International', '#28a745', 'lead', 1),
('Logistique', '#ffc107', 'lead', 1),
('Entreposage', '#6f42c1', 'lead', 1),
('Client Premium', '#fd7e14', 'opportunity', 1),
('Urgent', '#dc3545', 'general', 1),
('Suivi Régulier', '#17a2b8', 'general', 1);
