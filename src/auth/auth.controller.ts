...
export class AuthController {
  ...
  /**
   * Enregistrer une empreinte biométrique
  ...
  async registerBiometric(
    @Request() req,
    @Body() biometricData: { biometricHash: string }
  ) {
    try {
      const user = req.user as AuthenticatedUser;
      
      if (!biometricData.biometricHash) {
        throw new BadRequestException('Données biométriques manquantes');
      }

      const result = await this.authService.registerBiometric(
        user.id,
        user.userType,
  ...
  /**
   * Connexion avec empreinte biométrique
   * Nécessite userId et biometricHash
   */
  ...
  async loginWithBiometric(
    ...
    try {
      console.log('🔐 Tentative de connexion biométrique:', loginData.userId);

      if (!loginData.userId || !loginData.biometricHash || !loginData.userType) {
        throw new BadRequestException('Données de connexion incomplètes');
      }

      // Vérifier l'empreinte biométrique
      const result = await this.authService.verifyBiometric(
        loginData.userId,
        loginData.userType,
        loginData.biometricHash
      );
      ...
      const authResult = await this.authService.generateTokensForUser(
        result.user.id,
      ...
      response.cookie('refresh_token', authResult.refresh_token, cookieOptions);

      console.log('✅ Connexion biométrique réussie pour:', result.user.username);
      ...
      throw new UnauthorizedException(`Erreur authentification: ${error.message}`);
    }
  }
  ...
  async getBiometricStatus(@Request() req) {
    try {
      ...
      console.error('❌ Erreur récupération statut biométrique:', error);
      throw new BadRequestException(`Erreur: ${error.message}`);
    }
  }

  /**
   * Vérifier le statut biométrique d'un utilisateur par ID (sans authentification)
   * Utilisé pour afficher l'icône de biométrie sur la page de login
   */
  @Post('biometric/check-status')
  @HttpCode(HttpStatus.OK)
  async checkBiometricStatusByCredentials(
    @Body() checkData: { 
      usernameOrEmail: string;
    }
  ) {
    try {
      if (!checkData.usernameOrEmail) {
        throw new BadRequestException('Nom d\'utilisateur ou email requis');
      }

      const status = await this.authService.checkBiometricStatusByUsername(
        checkData.usernameOrEmail
      );
  ...
  async disableBiometric(@Request() req) {
    try {
      ...
      const result = await this.authService.disableBiometric(
        user.id,
        user.userType
      );
  ...
  /**
   * Générer un challenge biométrique pour la sécurité renforcée
   */
  @Get('biometric/challenge')
  @HttpCode(HttpStatus.OK)
  async generateBiometricChallenge() {
    try {
      const challenge = await this.authService.generateBiometricChallenge();
